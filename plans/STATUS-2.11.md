...
| Workers deployed (prod) | api-hotmart-ingress: NOVO deploy 2026-05-20 (Version ID `6f846f04`, rotação de token + reciclagem de isolates após 401 intermitente; 1 Secrets Store binding + Queue, 3 rotas fixas); **links-redirect: NOVO deploy 2026-05-19** (Version ID `64360b18`, multi-tenant via catálogo, sem env vars legadas — 2.11C.3); **dashboard-sync: NOVO deploy 2026-05-19** (Version ID `7a2aca8f`, 5 Secrets Store bindings, multi-tenant via catálogo); **funnel-dispatcher: NOVO deploy 2026-05-19** (Version ID `217c3c34`, 9 Secrets Store bindings, consumer queue ativo); **api-funnel-ingress: NOVO deploy 2026-05-19** (Version ID `5b8a689f`, 1 Secrets Store binding + Queue, CORS por catálogo) | 2026-05-20 |
...
- **2026-05-20 (Codex):** Incidente operacional pós-rotação de secret no `api-hotmart-ingress`: 401 intermitente em `PURCHASE_APPROVED` (mesmo token alternando entre 202/401), causado por cache de secret por isolate. Mitigação aplicada com redeploy do worker (Version ID `6f846f04`), reciclando isolates. Re-smoke autenticado: `12/12` sucesso (HTTP 202) em `decole-esg`, `planovoo` e `plano-de-voo`. Procedimento oficial atualizado: rotação de `HOTMART_WEBHOOK_TOKEN_DECOLE` exige redeploy na mesma janela.
