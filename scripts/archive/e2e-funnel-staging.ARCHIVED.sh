...
post_json() {
  ...
  if [[ -n "$auth_header" ]]; then
    status=$(curl -sS -o /tmp/e2e_response.json -w "%{http_code}" -X POST "$url" -H "content-type: application/json" -H "$auth_header" --data "$payload")
  else
...
wait_for_query_result() {
  ...
  local interval_secs="${5:-3}"
  local elapsed=0
  local result=""
  local value=""

  while (( elapsed < timeout_secs )); do
    result="$(d1_query_json "$db_name" "$sql")"
    value="$(echo "$result" | jq -r "$jq_expr")"
...
if [[ -n "$HOTMART_WEBHOOK_TOKEN" ]]; then
  HOTMART_HEADER="x-hotmart-hottok: ${HOTMART_WEBHOOK_TOKEN}"
fi
post_json "$HOTMART_INGRESS_URL/webhooks/v1/planovoo/hotmart/purchase" "{\"event_id\":\"$EVENT_ID_PA\",\"event\":\"PURCHASE_APPROVED\",\"buyer\":{\"email\":\"qa.auto.purchase@example.com\"},\"transaction\":\"txn-$EVENT_ID_PA\",\"created_at\":\"2026-04-22T23:00:00Z\",\"currency\":\"BRL\",\"value\":297.00}" "$HOTMART_HEADER"

echo "[step] aguardar processamento queue e consistencia D1"

echo "[step] validar event_store"
EVENT_RESULT=$(wait_for_query_result "$EVENT_DB_NAME" "SELECT event_id, profile_id, anonymous_id, event_type FROM funnel_events WHERE event_id='${EVENT_ID_GL}'" '.[0].results[0].event_id // empty' 60 3) || {
...
IDENTITY_RESULT=$(wait_for_query_result "$IDENTITY_DB_NAME" "SELECT profile_id, anonymous_id FROM identity_links WHERE anonymous_id='${ANON_ID}' ORDER BY updated_at DESC LIMIT 1" '.[0].results[0].profile_id // empty' 60 3) || {
